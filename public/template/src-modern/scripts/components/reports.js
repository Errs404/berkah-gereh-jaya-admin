...
document.addEventListener("alpine:init", () => {
    Alpine.data("reportsComponent", () => ({
        ...
        refreshData() {
            ...
            this.kpis.distributors = Math.floor(Math.random() * 200) + 800;
        ...
        generateReport() {
            this.showNotification(
                "New report generation would start here",
                "info"
            );
        },

        exportData() {
            const fileName = `report_${this.reportType}_${Date.now()}.${
                this.exportFormat
            }`;
            this.showNotification(
                `Exporting data as ${fileName}...`,
                "success"
            );
        },
        ...
        duplicateReport(report) {
            const newReport = {
                ...report,
                id: `RPT-${String(Math.floor(Math.random() * 1000)).padStart(
                    3,
                    "0"
                )}`,
        ...
        deleteReport(report) {
            if (confirm(`Are you sure you want to delete "${report.name}"?`)) {
                this.recentReports = this.recentReports.filter(
                    (r) => r.id !== report.id
                );
                this.showNotification(
        ...
        initCharts() {
            ...
            this.initDistributorAcquisitionChart();
                    ...
                    series: [
                        {
                            name: "Revenue",
                            data: [
                                28000, 32000, 35000, 41000, 38000, 45000, 52000,
                            ],
                        },
                        {
                            name: "Profit",
                            data: [
                                8400, 9600, 10500, 12300, 11400, 13500, 15600,
                            ],
                        },
                    ],
                    chart: {
        ...
        initDistributorAcquisitionChart() {
            const chartElement = document.getElementById(
                "distributorAcquisitionChart"
            );
            if (!chartElement) {
                console.warn("Distributor acquisition chart element not found");
                return;
            }
            ...
            try {
                const chartData = {
                    series: [
                        {
                            name: "New Distributors",
                            data: [23, 31, 45, 38, 52, 41, 67],
                        },
                        {
                            name: "Returning Distributors",
                ...
                console.error(
                    "Error rendering distributor acquisition chart:",
                    error
                );
            }
        },
