...
document.addEventListener("alpine:init", () => {
    Alpine.data("orderTable", () => ({
        ...
        calculateStats() {
            ...
            this.statusStats = Object.entries(statuses).map(
                ([name, count]) => ({
        ...
        filterOrders() {
            this.filteredOrders = this.orders.filter((order) => {
                ...
                const matchesDate =
                    !this.dateFilter || this.matchesDateFilter(order.orderDate);
        ...
        printInvoice(order) {
            ...
            this.showNotification(
                "Invoice would be generated and printed",
                "info"
            );
        },
        ...
        exportOrders() {
            const csvContent =
                ...
                "Order Number,Distributor,Email,Items,Total,Status,Date\n" +
                this.filteredOrders
                    .map(
                        (o) =>
                            `"${o.orderNumber}","${o.distributor.name}","${o.distributor.email}","${o.itemCount}","${o.total}","${o.status}","${o.orderDate}"`
                    )
                    .join("\n");
